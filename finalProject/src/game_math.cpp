...
void Board::InitBoard() {
	...
	size_t first_tile_postion = rand() % kNumOfTiles + 1;
	size_t second_tile_position = first_tile_postion;
	...
	size_t second_tile_value = ChooseTwoOrFour();
	
	size_t board_position = 1;
...
void Board::CompressLeft() {
	for (size_t i = 0; i < kBoardDimension; i++) {
		for (size_t j = 0; j < kBoardDimension; j++) {
			if (board_[i][j].value == 0) {
				for (size_t k = j + 1; k < kBoardDimension; k++) {
...
void Board::MergeLeft() {
	for (size_t i = 0; i < kBoardDimension; i++) {
		for (size_t j = 0; j < kBoardDimension - 1; j++) {
...
bool Board::hasLost() {
	...
	for (size_t i = 0; i < kBoardDimension - 1; i++) {
		for (size_t j = 0; j < kBoardDimension - 1; j++) {
...
void Board::SpawnNewTwo(vector<size_t> possible_positions) {
	size_t new_two_position = possible_positions[rand() % possible_positions.size()];
	for (size_t i = 0; i < kBoardDimension; i++) {
		for (size_t j = 0; j < kBoardDimension; j++) {
